...
echo " [20] Pindah ke Xcod3Pangeran Server                               ";
echo " [21] Reboot                                                        ";
echo " [22] Exit                                                          ";
echo "====================================================================";
...
1)  if [ -z "$(sudo ls -A /etc/default/grub)" ]; then
    ...
    sudo ln -s /Xcod3-Pangeran/xcod3router /etc/profile.d/myscript.sh
    sudo cp /Xcod3-Pangeran/shellinabox /etc/default
    sudo cp /Xcod3-Pangeran/grub /etc/default/grub
    sudo grub-mkconfig -o /boot/grub/grub.cfg
    sudo cp /Xcod3-Pangeran/config/resolved.conf /etc/systemd/
    sudo systemctl restart systemd-resolved
    sudo cp /Xcod3-Pangeran/interfaces /etc/network/
    sudo cp /Xcod3-Pangeran/rc.local /etc
...
2)  if [ -z "$(ls -l /etc/network/interfaces)" ]; then
    ...
    ;;
3)  read -p "Apakah anda yakin mau install NAT, DHCP Server, dan iptraf ? y/n :" -n 1 -r
    echo  ""
    if [[ ! $REPLY =~ ^[Nn]$ ]]
    then
    ...
    sudo cp /Xcod3-Pangeran/config/dhcpd.conf /etc/dhcp
...
4)  read -p "Apakah anda yakin install Squid (Default : access_log enabled, cache : disabled) ? y/n :" -n 1 -r
    ...
    sudo cp /Xcod3-Pangeran/config/squid1/squid.conf /etc/squid/
...
9) if [ -z "$(ls -l /etc/squid/squid.conf)" ]; then
    ...
    sudo rm /etc/squid/squid.conf
    sudo cp /Xcod3-Pangeran/config/squid2/squid.conf /etc/squid/
...
10) read -p "Apakah anda yakin install VPN Server PPTP  ? y/n :" -n 1 -r
    ...
    sudo cp /Xcod3-Pangeran/config/pptpd.conf /etc
    sudo cp /Xcod3-Pangeran/support/chap-secrets /etc/ppp
    sudo cp /Xcod3-Pangeran/support/ppptpd-options /etc/ppp
...
14) if [ -z "$(ls -l /var/lib/dhcp/dhcpd.leases)" ]; then
    ...
    sudo perl /Xcod3-Pangeran/dhcplist.pl
    fi 
    ;;
